var oembed = {
  ...
  trim_text: function(text) {
    ...
    return (match ? match[0] : text.substr(0, 156)) + ' ...';    
  },
  templates: APPS.core.ddoc.vendor.tadagraph.templates.oembed,
  embed: {
    video: function(oembed_result) {
      return oembed_result.html;
    },
    rich: function(oembed_result) {
      return oembed_result.html;
    },
    photo: function(oembed_result) {
      return $.mustache('<img src={{url}} height={{height}} width={{width}} />',
                        oembed_result);
    }
    ...
    links.each(function() {
      ...
      if (url_re.test(href)) {
        var oembed_url = 'http://api.embed.ly/1/oembed';
                 ...
                 !/^http:\/\/([^\/]*\.?)tadagraph\.com/.test(href) &&
                 !/^http:\/\/([^\/]*\.?)tadagr\.com/.test(href)) {
        // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        // !!!!!!!!!!!!!!!!!!!!!!!!!! ATTENTION !!!!!!!!!!!!!!!!!!!!!!!!!!!
        // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

        // CHANGE THIS TO API URL!!! When it will be live

        var oembed_url = 'http://tadagr.com/app/screenshoter/oembed';
      ...
      if (!oembed_url) return;

      var that = $(this);
      
      if (oembed.cache[this.href]) return success(oembed.cache[this.href]);
      ...
      $.ajax({
        ...
        success: function(result) {
          oembed.cache[this.href] = result;
          success(result);
        }
      });
      function success(oembed_result) {
        if (!oembed_result || !oembed.templates[oembed_result.type]) return;
        
        oembed_result.thumbnail_url = oembed_result.thumbnail_url ||
                                      oembed_result.url;
        
        oembed_result.description =
            oembed.trim_text(oembed_result.description || '');
        
        var embed_html = oembed.embed[oembed_result.type](oembed_result),
            preview_html = $($.mustache(oembed.templates[oembed_result.type],

                                        $.extend(true, oembed_result, {
                                          original_url: href
                                        })));
        
        that.parent('span.text:first').after(preview_html);
        that.remove();
        
        preview_html.find('a.oembed-original-url').click(function(event) {            
          event.preventDefault();
          event.stopPropagation();

          embed_html = $(embed_html);
          embed_html.find('embed').attr('wmode', 'opaque');
  ...
  sandbox: function(oembed_result) {
    var html = [
        ...
        size = oembed_result.width || oembed_result.height;
      
      return '<iframe class="oembed-sandbox" ' +
             'src="data:text/html;charset=utf-8;plain,' +
             encodeURIComponent(html) + '" ' + 
             (size ? 'style="width: ' + oembed_result.width + 'px;' +
                     'height: ' + oembed_result.height + 'px;"' : '') + '/>';
  }
};

if (typeof exports !== 'undefined') {
  exports.wrap = oembed.wrap;
  exports.sandbox = oembed.sandbox;
  exports.templates = oembed.templates;
