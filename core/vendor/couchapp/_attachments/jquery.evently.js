      ...
      all: function() {
        ...
        try {
          ...
          return keys.map(function(key) {
            try {
              return {key: key, value: JSON.parse(storage[key])};
            } catch(e) {
...
(function($) {
    ...
    connect : function(source, target, events) {
      events.forEach(function(ev) {
        $(source).bind(ev, function() {
          ...
          //$.log("source:", source, "target:", $(target), ev);
          args.shift();
  ...
  */
  ...
  // todo this should return the new element
  function mustachioed(me, h, args) {
    var context = runIfFun(me, h.data, args);
    
    var benchmark = (context && context.benchmark);
    
    if (benchmark)
        startBenchmark(context.benchmark);
        
    var partials = $$(me).partials;
    var a1 = runIfFun(me, h.mustache, args);
    var a3 = runIfFun(me, $.extend(true, partials, API.partials, h.partials), args);
    
    if (benchmark)
        measureBenchmark(context.benchmark);
    
    var result = $($.trim($.mustache(a1, context, a3)));
